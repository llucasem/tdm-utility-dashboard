...
export function mapBillRow(row) {
  ...
  // Fecha del PAGO real, cuando el matcher caso la factura con una
  // transaccion de QuickBooks. Es el segundo eje de mes: el cierre de Jake
  // va por cuando salio el dinero, no por cuando llego la factura.
  const paidStr  = row.qb_match_status === 'matched'
    && Array.isArray(row.qb_match_data) && row.qb_match_data[0]?.date
    ? row.qb_match_data[0].date : null;
