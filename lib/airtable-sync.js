...
async function insertRentPayment(email, v) {
  let property = v.property_address || null;
  let unit     = v.unit             || null;
  if (property && OFFICE_ADDRESSES.some(re => re.test(property))) {
    property = null;
  }

  // 1.5. Alias map (rent_alias_map) — mailbox + amount, derived from months of
  // Jake's own QB Classes. Most reliable automatic source; conservative on
  // ambiguity (returns null → falls through to the next sources / review).
  if (!property || !unit) {
    const r = await resolveRentAlias({ mailbox: email.mailbox, amount: v.amount_paid, landlord: v.landlord });
    if (r) {
