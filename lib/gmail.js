...
export async function getUtilityEmails() {
  ...
  // 3. Filter against the DB — skip emails already seen. This is what makes
  //    the cron fast: it doesn't re-fetch 400+ already-processed emails.
  //    Se consultan las DOS tablas: processed_emails es el registro nuevo de
  //    "esto ya lo he mirado" (incluido el ruido), y utility_bills se mantiene
  //    porque el historial anterior al reset solo consta ahi.
  const knownR = await pool.query(
    `SELECT gmail_message_id FROM processed_emails WHERE gmail_message_id = ANY($1::text[])
     UNION
