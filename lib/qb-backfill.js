...
export async function backfillBillsFromQB({ sinceDays = 60, maxCreates = 30 } = {}) {
  ...
  for (const p of purchases) {
    ...
    // CADA pago de un proveedor de servicios se registra como HECHO en
    // `payments`, se case o no con una factura. Sin esto, la cola de revision
    // no tiene candidatos que ofrecer: los pagos que no casan exactos — donde
    // vive justo el "$75 de la factura de $100" de Jake — no existian en
    // ninguna tabla. Registrar un hecho no asigna nada: la asignacion sigue
    // siendo del matcher exacto o de una persona.
    if (Number(p.TotalAmt) > 0 && p.TxnDate) {
      try {
        const c0 = cls.topClass || cls.lineClasses?.[0] || null;
        const pr = await pool.query(`
          ...
          ON CONFLICT (qb_purchase_id) DO NOTHING
          RETURNING 1`,
          [String(p.Id), p.TxnDate, Number(p.TotalAmt), p.EntityRef?.name ?? null,
           c0?.value != null ? String(c0.value) : null, c0?.name ?? null,
           p.AccountRef?.name ?? null,
           JSON.stringify({ id: p.Id, date: p.TxnDate, amount: Number(p.TotalAmt), doc: p.DocNumber ?? null })]);
        if (pr.rowCount) stats.pagos_registrados = (stats.pagos_registrados || 0) + 1;
