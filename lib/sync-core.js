...
export function resolverPropiedad(registry, item) {
  ...
  if (item.service_address) {
    return {
      address: normAddress(item.service_address),
  ...
  return { address: null, unit: null, origen: 'sin asignar', discrepancia: null };
}
...
export async function aprenderCuenta(db, registry, item, prop) {
  if (!prop.address || !item.account_last4) return false;
  await db.query(
    `INSERT INTO account_registry
       (utility_type, account_last4, provider, property_address, unit, confidence,
        bills_seen, notes, first_seen_at, last_seen_at)
    ...
    [item.utility_type, item.account_last4, item.provider ?? null,
     prop.address, prop.unit, 'Aprendida del propio email del proveedor durante el sync.']
  );
