...
export async function parseEmail(email) {
  ...
  const response = await client.messages.create({
    ...
    // Per-call ceiling so one slow parse (e.g. a large PDF) can't eat the
    // whole sync function's 60s Vercel budget. The sync route defers any
    // emails it doesn't reach to the next run.
    timeout: 25_000,
    maxRetries: 1,
  });
