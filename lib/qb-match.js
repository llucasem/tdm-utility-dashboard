...
export async function matchBill(bill) {
  ...
  // Purchase we'd be double-attributing it. The claimed candidates are kept
  ...
  const stored = unclaimed.slice(0, MAX_STORED_MATCHES);

  let status;
  if (unclaimed.length === 0)      status = 'not_found';
  else if (unclaimed.length === 1) status = 'matched';
  else                              status = 'ambiguous';

  await persist({ billId, status, count: unclaimed.length, data: stored });
  return { status, count: unclaimed.length, matches: stored, claimedCount };
}
