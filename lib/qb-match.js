...
export async function matchBill(bill) {
  ...
  // Claim filter — drop any Purchase that already has a Class in QuickBooks.
  // Reason: a Class means the owner (Jake) has already attributed that
  // transaction to a specific property. If we matched a new bill to a claimed
  // Purchase we'd be double-attributing it.
  const unclaimed = matches.filter(m => !m.hasClass);

  // Vendor filter — require the Purchase payee to match the bill's expected
  // utility vendor (inferred from email_from). Without this, a $100 payment
  // to "Yaritza" would falsely match a $100 internet bill from Spectrum.
  const expectedVendors = inferBillVendor(bill.email_from);
  let vendorChecked;
  if (expectedVendors) {
    vendorChecked = unclaimed.filter(m => vendorMatches(expectedVendors, m.payee));
  } else {
    // Unknown vendor — keep all unclaimed candidates but the bill is
    // inherently low-confidence. We'll still return 'ambiguous' below if
    // there's more than one, and never auto-tag without manual review.
    vendorChecked = unclaimed;
  }

  const claimedCount   = matches.length - unclaimed.length;
  const vendorRejected = unclaimed.length - vendorChecked.length;
  const stored = vendorChecked.slice(0, MAX_STORED_MATCHES);

  let status;
  if (vendorChecked.length === 0) status = 'not_found';
  else if (vendorChecked.length === 1 && expectedVendors) status = 'matched';
  else                              status = 'ambiguous';
  // ↑ note: if vendor was inferred AND exactly 1 candidate survives both
  // filters → 'matched'. If vendor was NOT inferred and 1 candidate exists,
  // we downgrade to 'ambiguous' so a human reviews it.

  await persist({ billId, status, count: vendorChecked.length, data: stored });
  return { status, count: vendorChecked.length, matches: stored, claimedCount, vendorRejected };
}
