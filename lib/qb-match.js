...
async function getExpectedClass(bill) {
  ...
  return all.rows.find(r => addressesMatch(r.property_address, bill.property_address) && normalizeUnit(r.unit) === u) || null;
}
...
async function getClassProperties(classIds) {
  ...
  const r = await pool.query(
    `SELECT qb_class_id, property_address, unit FROM property_qb_class WHERE qb_class_id::text = ANY($1::text[])`,
    [classIds]
  );
  return new Map(r.rows.map(x => [String(x.qb_class_id), x]));
}
