...
async function persist({ billId, status, count = null, data = null, error = null }) {
  ...
  // Paso 3 del plan: ademas del JSON en la factura, el pago se guarda como
  // HECHO en `payments` y el vinculo como asignacion en `bill_payments`.
  // De ahi se derivan paid_amount/paid_date — siempre recalculables.
  if (status === 'matched' && count === 1 && data?.[0]) {
    try { await recordAllocation(billId, data[0]); }
    catch (e) { console.error('[qb-match] allocation failed:', e.message); }
  }
}

async function recordAllocation(billId, m) {
  if (!m?.id || !m?.date || !(Number(m.amount) > 0)) return;
  const p = await pool.query(`
    ...
    RETURNING id`,
    [String(m.id), m.date, m.amount, m.payee ?? null, m.classId ?? null,
     m.className ?? null, m.account ?? null, JSON.stringify(m)]);
  const paymentId = p.rows[0].id;

  // Un pago solo salda UNA factura salvo decision humana. Si otra factura ya
  // lo reclama, no se pisa: ese caso es de la cola de revision, no del
  // matcher. (El reclamo por orden de llegada fue una de las criticas del
  // /roast — aqui el empate queda visible en vez de resolverse solo.)
  const taken = await pool.query(
    `SELECT bill_id FROM bill_payments WHERE payment_id = $1 LIMIT 1`, [paymentId]);
  if (taken.rowCount > 0 && taken.rows[0].bill_id !== billId) return;

  await pool.query(`
    INSERT INTO bill_payments (bill_id, payment_id, allocated_amount, source)
    VALUES ($1,$2,NULL,'exact-match')
    ON CONFLICT (bill_id, payment_id) DO NOTHING`, [billId, paymentId]);
}
