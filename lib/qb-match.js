...
export async function matchBill(bill) {
  ...
  // Asymmetric window: a Purchase for THIS bill can be up to 3 days BEFORE
  // the email (autopay charged just before the statement was issued) or up
  // to 30 days AFTER (late manual payment). Anything >3 days before belongs
  // to a previous billing cycle and is not this bill's payment.
  const dateFrom  = shiftDate(anchorIso, -3);
  ...
  const unclaimed = vendorFiltered;
  const claimedCount = claimedByQB + claimedByOurBills + vendorMismatchCount;
