...
export async function refreshLearnedVendors({ sinceDays = 180 } = {}) {
  ...
  // CRITICAL: only learn from Jake-confirmed bills, NOT from auto-tagged bills.
  // Otherwise a mis-tag from the system creates a feedback loop where its own
  // mistake gets promoted to "known vendor". A bill counts as Jake-confirmed if
  // it has an entry in class_learning_log with action IN ('confirmed',
  // 'synced_from_qb', 'relinked') — those actions only happen when Jake himself
  // assigned a Class in QB.
  const r = await pool.query(`
    WITH jake_confirmed_bills AS (
      SELECT DISTINCT bill_id
      FROM class_learning_log
      WHERE action IN ('confirmed', 'synced_from_qb', 'relinked')
        AND created_at > NOW() - ($1 * INTERVAL '1 day')
    ),
    parsed AS (
      SELECT b.utility_type,
             LOWER(TRIM(REGEXP_REPLACE(((b.qb_match_data->0)->>'payee')::text, '[^a-zA-Z0-9 &]', '', 'g'))) AS payee
      FROM utility_bills b
      JOIN jake_confirmed_bills jcb ON jcb.bill_id = b.id
      WHERE b.qb_tag_status = 'tagged'
        AND b.qb_match_data IS NOT NULL
        AND jsonb_typeof(b.qb_match_data) = 'array'
        AND jsonb_array_length(b.qb_match_data) >= 1
        AND ((b.qb_match_data->0)->>'payee') IS NOT NULL
    )
...
export async function refreshLearnedBankAccounts({ sinceDays = 180 } = {}) {
  const stats = { processed: 0, promoted: 0, new: 0 };
  // Same Jake-confirmed filter as refreshLearnedVendors above.
  const r = await pool.query(`
    WITH jake_confirmed_bills AS (
      SELECT DISTINCT bill_id
      FROM class_learning_log
    ...
    parsed AS (
      SELECT LOWER(TRIM(((b.qb_match_data->0)->>'account')::text)) AS account
      FROM utility_bills b
      JOIN jake_confirmed_bills jcb ON jcb.bill_id = b.id
      WHERE b.qb_tag_status = 'tagged'
        AND b.qb_match_data IS NOT NULL
        AND jsonb_typeof(b.qb_match_data) = 'array'
        AND jsonb_array_length(b.qb_match_data) >= 1
        AND ((b.qb_match_data->0)->>'account') IS NOT NULL
    )
