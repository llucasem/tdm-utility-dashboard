...
export async function searchTransactions({ amount, dateFrom, dateTo }) {
  ...
  // We hit Purchase + BillPayment in parallel. Purchase needs SELECT * so the
  // response includes ClassRef + Line[] (QBO's query language doesn't allow
  // explicit ClassRef in the field list).
  const [purchases, billPayments] = await Promise.all([
    queryQB(`SELECT * FROM Purchase ${where}`).catch(() => null),
