 ...
 */
// Advisory lock key derived from a stable, project-specific constant. Two
// concurrent Vercel invocations refreshing QB tokens would otherwise both POST
// to Intuit, the second of which invalidates the first refresh_token and locks
// the whole app out of QB. With this lock, only one process refreshes; the
// other waits and re-reads the row.
const QB_REFRESH_LOCK_KEY = 8429471;
...
export async function getValidTokens() {
  ...
  if (expiresInMs >= REFRESH_BUFFER_SECONDS * 1000) return row;

  // Need refresh — serialize with advisory lock to avoid the race.
  const client = await pool.connect();
  try {
    await client.query('SELECT pg_advisory_lock($1)', [QB_REFRESH_LOCK_KEY]);
    // Re-read: another invocation may have just refreshed.
    const fresh = await client.query(
      `SELECT realm_id, access_token, refresh_token, expires_at, refresh_expires_at
       FROM quickbooks_tokens ORDER BY updated_at DESC LIMIT 1`
    );
    const reloaded = fresh.rows[0];
    const reloadedExpiresInMs = new Date(reloaded.expires_at).getTime() - Date.now();
    if (reloadedExpiresInMs >= REFRESH_BUFFER_SECONDS * 1000) {
      return reloaded;
    }
    return await refreshAccessToken(reloaded);
  } finally {
    await client.query('SELECT pg_advisory_unlock($1)', [QB_REFRESH_LOCK_KEY]).catch(() => {});
    client.release();
  }
}
...
/**
 * Extract Class info from a Purchase (or BillPayment, though those rarely have Class).
 * Used by both the matcher (claim filter) and the auto-tag guardrail.
 *
 * Returns { topClass, lineClasses, hasClass } where:
 *  - topClass   = ClassRef at the top level, or null
 *  - lineClasses = array of ClassRef objects from each line (filtered to non-null)
 *  - hasClass   = true if any Class is set anywhere
 */
export function extractClassInfo(purchase) {
 ...
 *
 * Each returned match includes Class info so callers can implement claim
 * filters (skip Purchases that already have a Class — they're reclaimed by
 * the QuickBooks owner and should not be matched to new bills).
 *
 * @param {object} opts
...
export async function searchTransactions({ amount, dateFrom, dateTo }) {
  ...
  // Pagination helper — QBO default MAXRESULTS is 100, must page via STARTPOSITION.
  async function paginate(selectClause) {
    const all = [];
    const pageSize = 100;
    let pos = 1;
    while (true) {
      const r = await queryQB(`${selectClause} ${where} STARTPOSITION ${pos} MAXRESULTS ${pageSize}`).catch(e => {
        console.error(`[searchTransactions] page ${pos} failed: ${e.message}`);
        return null;
      });
      if (!r) break;
      const key = Object.keys(r.QueryResponse || {}).find(k => Array.isArray(r.QueryResponse[k]));
      const items = key ? r.QueryResponse[key] : [];
      all.push(...items);
      if (items.length < pageSize) break;
      pos += items.length;
      if (pos > 1000) break; // safety cap
    }
    return all;
  }

  // We hit Purchase + BillPayment in parallel.
  const [purchases, billPayments] = await Promise.all([
    paginate('SELECT * FROM Purchase'),
    paginate('SELECT Id, TxnDate, TotalAmt, VendorRef, PrivateNote, DocNumber FROM BillPayment'),
  ]);
  ...
  for (const p of purchases) {
    ...
    matches.push({
      ...
      classId:    cls.topClass?.value || cls.lineClasses[0]?.value || null,
      className:  cls.topClass?.name  || cls.lineClasses[0]?.name  || null,
  ...
  for (const bp of billPayments) {
    matches.push({
      ...
      classId:    null,    // BillPayment is settlement, doesn't carry Class
      className:  null,
      hasClass:   false,
    });
  }
...
export async function tagPurchaseWithClass({ purchaseId, classId, className }) {
  ...
  if (cls.hasClass) {
    return {
      ...
      previousClass: {
        topClass:    cls.topClass?.value || null,
        lineClasses: cls.lineClasses.map(c => c.value),
      },
    };
  }
