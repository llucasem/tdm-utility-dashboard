...
import { normalizeAddress, normalizeUnit } from '@/lib/address-normalize';
...
export async function linkBillsFromRecentClasses({ sinceDays = 14 } = {}) {
  ...
  for (const purchase of classed) {
    ...
    let target = null;
    if (mapping) {
      // Use the address normalizer so "St" vs "Street", "Ave" vs "Avenue",
      // missing city suffix, etc., compare equal. Without this many bills
      // never matched (e.g. "175 W 107th" vs "175 W 107th St").
      const mapAddr = normalizeAddress(mapping.property_address);
      const mapUnit = normalizeUnit(mapping.unit);
      const matches = candidates.rows.filter(b => {
        return normalizeAddress(b.property_address) === mapAddr
      ...
      if (matches.length === 1) target = matches[0];
        ...
        // No bill with matching property by mapping. Fallback order:
        //   1. Exactly one Unassigned candidate → take it
        //   2. Exactly one candidate (any property) → take it (amount unique)
        ...
        else if (candidates.rowCount === 1) target = candidates.rows[0];
      }
    } else {
