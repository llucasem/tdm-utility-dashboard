...
import { isKnownVendor } from '@/lib/known-vendors';
import { createNotification } from '@/lib/notifier';
import { normalizeUnit, addressesMatch } from '@/lib/address-normalize';
...
export async function learnFromClassedPurchase(purchase) {
  ...
  // Find utility_bills with same amount. Window is asymmetric and anchored
  // on email_received_at: a bill is candidate if its email arrived from
  // 30d before to 10d after the Purchase date. Anything else is from a
  // different billing cycle. (Per project rule: la fecha de una factura
  // es la del email, no la due_date.)
  const dateFrom = shiftDate(txnDate, -30);
  const dateTo   = shiftDate(txnDate,  10);
  const candidates = await pool.query(`
    SELECT id, property_address, unit, amount_due, due_date, email_received_at
    FROM utility_bills
    WHERE ROUND(amount_due::numeric, 2) = ROUND($1::numeric, 2)
      AND email_received_at::date BETWEEN $2::date AND $3::date
...
export async function runLearningPass({ sinceDays = 60 } = {}) {
  ...
  // STARTPOSITION is a 1-based offset, not a page number — increment by
  // items.length, not by page size.
  const allPurchases = [];
  let pos = 1;
  while (true) {
  ...
  if (stats.created > 0 || stats.conflicts > 0) {
    const parts = [];
    if (stats.created > 0)   parts.push(`✓ ${stats.created} new mappings inferred`);
    ...
    await createNotification({
      ...
      title:   `Auto-learn · ${classed.length} classed Purchases processed`,
      message: parts.join(' · '),
...
export async function linkBillsFromRecentClasses({ sinceDays = 14 } = {}) {
  ...
  // STARTPOSITION is a 1-based offset, not a page number — increment by items.length.
  const allPurchases = [];
  let pos = 1;
  while (true) {
    const q = await queryQB(`SELECT * FROM Purchase WHERE TxnDate >= '${since}' STARTPOSITION ${pos} MAXRESULTS 500`);
  ...
  for (const purchase of classed) {
    ...
    const txnDate   = purchase.TxnDate;
    const amount    = Number(purchase.TotalAmt);
    // Asymmetric window from Purchase perspective: a bill is candidate if
    // its email_received_at is within [Purchase - 30, Purchase + 10] days.
    // Reason: for a given Purchase, the corresponding email arrives 0-10
    // days before (autopago) or up to 30 days after (slow notification).
    // If the email is too long AFTER the Purchase, it's the next cycle.
    const dateFrom  = shiftDate(txnDate, -30);
    const dateTo    = shiftDate(txnDate,  10);

    // Find candidate bills — anchored on email_received_at, NOT due_date.
    // Per project rule: la fecha de una factura es la del email.
    // Never link to hidden duplicate rows: linking the invisible twin left
    // the visible bill at ✗ forever while stats said "already correct".
    const candidates = await pool.query(`
      SELECT id, property_address, unit, amount_due, due_date, email_received_at,
             utility_type, qb_purchase_id, qb_tag_status, qb_match_status
      FROM utility_bills
      WHERE ROUND(amount_due::numeric, 2) = ROUND($1::numeric, 2)
        AND email_received_at::date BETWEEN $2::date AND $3::date
        AND NOT is_duplicate
    ...
    let target = null;
    if (mapping) {
      // Use the address matcher so "St" vs "Street", "Ave" vs "Avenue",
      // missing city suffix, comma-less addresses, etc., compare equal.
      // Without this many bills never matched (e.g. "175 W 107th" vs
      // "175 W 107th St", "4250 Glencoe Ave Marina del Rey CA 90292").
      const mapUnit = normalizeUnit(mapping.unit);
      const matches = candidates.rows.filter(b => {
        return addressesMatch(b.property_address, mapping.property_address)
            && normalizeUnit(b.unit) === mapUnit;
      });
      if (matches.length === 1) target = matches[0];
        ...
        // No bill matches the mapping's property. If the only plausible
        // candidate is UNASSIGNED, adopt it (parser couldn't read the
        // address; the Class fills it in). Do NOT link a candidate that
        // carries a DIFFERENT property — that produced cross-property links
        // ("939 #408" onto a 607 2nd Ave bill, same $65 Spectrum amount).
        // Vendor guard: the Class can't verify the property here, so at
        // least the payee must fit the bill's utility_type.
        const unassigned = candidates.rows.filter(b => !b.property_address || !b.property_address.trim());
        if (unassigned.length === 1 && await isKnownVendor(unassigned[0].utility_type, purchase.EntityRef?.name)) {
          target = unassigned[0];
        }
      }
    } else {
      // No mapping for this Class — we can't verify the property. Only adopt
      // when the single candidate is UNASSIGNED (nothing to contradict) and
      // the payee fits the bill's utility_type. Linking assigned bills here
      // produced cross-property mis-links (e.g. "360 W Pico" internet onto a
      // 607 2nd Ave bill).
      if (candidates.rowCount === 1) {
        const only = candidates.rows[0];
        const isUnassigned = !only.property_address || !only.property_address.trim();
        if (isUnassigned && await isKnownVendor(only.utility_type, purchase.EntityRef?.name)) target = only;
      }
    }
