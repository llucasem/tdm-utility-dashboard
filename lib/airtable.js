 ...
 */
export async function getUnprocessedEmails({ limit = 25, maxScan = 3000 } = {}) {
 ...
 * so we never re-classify it. Retries on transient DNS / connection errors
 * (Neon free-tier endpoint can flake during long backfills).
 */
export async function markProcessed(airtableId, verdict, reason = null) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      lastErr = e;
      const transient = e.code === 'ENOTFOUND' || e.code === 'ECONNRESET' || e.code === 'ETIMEDOUT';
