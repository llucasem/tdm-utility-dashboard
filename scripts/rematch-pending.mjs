/**
 * Match all bills currently stuck in qb_match_status='pending'.
 * Mirrors the logic of lib/qb-match.js (anchor email_received_at, window
 * -10/+30, claim filter, bill-purchase exclusivity, historical disambig).
 *
 * Run with:  node scripts/rematch-pending.mjs [--dry-run]
 */
...
async function qb(sql) {
  const r = await fetch(
    `https://quickbooks.api.intuit.com/v3/company/${tok.realm_id}/query?query=${encodeURIComponent(sql)}&minorversion=70`,
...
async function getHistoricalPattern(property_address, unit, utility_type) {
  ...
  const r = await pool.query(`
    ...
    ORDER BY email_received_at DESC NULLS LAST LIMIT 6
...
async function disambiguate(bill, candidates) {
  ...
  return { candidate: w.c, score: w.score, runnerUp: r?.score ?? null };
}
...
async function matchOne(bill) {
  ...
  // Claim filter (Jake's classed Purchases already attributed)
  const unclaimedByQB = candidates.filter(c => !c.hasClass);

  // Bill-purchase exclusivity
  const ids = unclaimedByQB.map(c => c.id);
  let trulyUnclaimed = unclaimedByQB;
  if (ids.length > 0) {
    const taken = await pool.query(`
      SELECT DISTINCT (jsonb_array_elements(qb_match_data)->>'id') AS taken_id
      FROM utility_bills
      WHERE id != $1
        AND qb_match_status = 'matched'
        AND qb_match_data IS NOT NULL
    `, [bill.id]);
    const takenSet = new Set(taken.rows.map(r => r.taken_id));
    trulyUnclaimed = unclaimedByQB.filter(c => !takenSet.has(String(c.id)));
  }

  let status, stored;
  if (trulyUnclaimed.length === 0) { status = 'not_found'; stored = []; }
  else if (trulyUnclaimed.length === 1) { status = 'matched'; stored = trulyUnclaimed; }
  else {
    const pick = await disambiguate(bill, trulyUnclaimed);
    if (pick) {
      status = 'matched';
      stored = [{ ...pick.candidate, disambiguation: { reason: 'history', score: pick.score, runnerUpScore: pick.runnerUp } }];
    } else {
      status = 'ambiguous';
      stored = trulyUnclaimed.slice(0, 20);
    }
  }

  return { status, count: stored.length, matches: stored };
}

// ── Main ─────────────────────────────────────────────────────────────
console.log(`\nREMATCH-PENDING  ${DRY_RUN ? '(DRY RUN)' : '(WRITING)'}\n`);

const bills = await pool.query(`
  SELECT id, amount_due, due_date, email_received_at, email_from,
         property_address, unit, utility_type
  FROM utility_bills
  WHERE qb_match_status = 'pending' AND amount_due > 0
  ORDER BY id
`);
console.log(`Bills en pending: ${bills.rowCount}\n`);

const summary = { matched: 0, not_found: 0, ambiguous: 0, skipped: 0, errors: 0 };

for (let i = 0; i < bills.rows.length; i++) {
  const b = bills.rows[i];
  process.stdout.write(`\r  ${i + 1}/${bills.rowCount}  matched=${summary.matched}  not_found=${summary.not_found}  ambiguous=${summary.ambiguous}  errors=${summary.errors}    `);
